...
{
    WeightedTree* tmp;
    Queue* queue = CreateQueue();   // Use queue to traverse the tree in level order
    Enqueue(queue, tree);
    ...
    while (queue->tail)//queue is not empty
    {
        ...
        if (tmp->weight == -1 && queue->head)   // If this node is the level node, that is, the end of the level
        {
            depth++;
            Enqueue(queue, level);
        }
    ...
    // If the wpl of this tree is the same with optimalWpl got by Huffman algorithm,
    // this should be an optimal encoding way as we have set wpl of code not satisfying prefix rule to INF.
...
int GetWPL(WeightedTree* tree)
{
    ...
    WeightedTree* level = (WeightedTree *)malloc(sizeof(WeightedTree)); // Denote end of one level
    level->left = level->right = NULL;
    level->weight = -1;
