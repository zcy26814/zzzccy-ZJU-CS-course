...
void StartFilledRegion(double density);//填充开始 
void EndFilledRegion(void);//填充结束 
...
void DrawTextString(string text);//输入 
...
double TextStringWidth(string text);//获取字符串宽度 
...
void SetFont(string font);//设置新字体 
...
void UpdateDisplay(void);//此功能启动图形窗口的立即更新，并且对于动画是必需的。
                         //通常，仅在程序等待用户输入时才更新图形窗口。 
...
void Pause(double seconds);//暂停功能会更新图形窗口，然后暂停指定的秒数。
                           //此功能对于否则运动会太快的动画很有用。
...
void ExitGraphics(void);//ExitGraphics函数关闭图形窗口并退出应用程序
                        //而无需等待任何其他用户交互。 
...
void SaveGraphicsState(void);//SaveGraphicsState函数在内部保存当前图形状态
                             //（当前笔的位置，字体，磅值和擦除模式标志）
                             //以便可以通过下一个RestoreGraphicsState调用来还原它们。
                             //这两个函数必须成对使用，但可以嵌套到任何深度。 
